...
// GetForecast handles GET /api/weather/forecast?province=xxx&amphoe=yyy&duration=7
func (h *WeatherHandler) GetForecast(c *fiber.Ctx) error {
	...
	amphoe := c.Query("amphoe", "")
	duration, _ := strconv.Atoi(c.Query("duration", "7"))

	forecast, err := h.svc.GetForecast(province, amphoe, duration)
	if err != nil {
...
// GetProvinces returns list of all Thai provinces
func (h *WeatherHandler) GetProvinces(c *fiber.Ctx) error {
